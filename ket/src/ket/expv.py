...
class Pauli(HasProcess):
    ...
    def __init__(  # pylint: disable=too-many-arguments
        ...
        *,
        ...
        if pauli is not None and qubits is not None:
            ...
            self.map = {q: pauli for q in qubits.qubits} if pauli != "I" else {}
            self.coef = 1.0
        else:
            ...
            self.map = {q: p for q, p in _map.items() if p != "I"}
            self.coef = _coef
    ...
    def _str_no_coef(self) -> str:
        return "".join(f"{pauli}{qubit}" for qubit, pauli in sorted(self.map.items()) if pauli != "I")
    ...
    def __str__(self) -> str:
        ...
        op = " ".join(
            f"{p}({q})"
            for q, p in sorted(self.map.items())
            if p != "I"
        )
...
class Hamiltonian(HasProcess):
    ...
    def __str__(self) -> str:
        ...
        return " + ".join(str(p) for p in self.terms)
